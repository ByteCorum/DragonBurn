...
void Offsets::UpdateOffsets()
{
    ...
    // Check info.json build number - if bumped, force re-download
    std::string gameBuildNum = std::to_string
    (
        json::parse(Web::Get("https://raw.githubusercontent.com/some-random-guy-ah/cs2-offsets/refs/heads/main/info.json"))["build_number"]
        .get<int>()
    );
    ...
    bool buildMatches = GamaDataStorage.contains("build-number") &&
        GamaDataStorage["build-number"].get<std::string>() == gameBuildNum;

    if (!buildMatches)
    {
        // info.json version changed - you pushed new offsets, download from a2x regardless of force_local
        Log::Info("Fetching offsets from GitHub...");
    ...
    {
        // Build matches - use local files (either user-dumped or previously cached from a2x, same path either way)
        Log::Fine("Build matches, using local offsets");
