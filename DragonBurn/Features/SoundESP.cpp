...
namespace SoundESP {
    ...
    static std::map<DWORD64, float> lastSoundTimes;
    static std::map<DWORD64, double> lastSpawnTimes;
    static std::map<unsigned int, float> animationStack;
    ...
    float GetAnimationValue(const std::string& key, float deltaTime) {
        ...
        auto it = animationStack.find(hash);
        
        if (it == animationStack.end()) {
            animationStack[hash] = 0.0f;
            return 0.0f;
        }
